$(function () {
    ...
    function scrollToTab(element) {
        ...
        // 可视区域非tab宽度
        var tabOuterWidth = calSumWidth($(".content-tabs").children().not(".menu-tabs"));
        //可视区域tab宽度
    ...
    function menuItem() {
        ...
        $('.menu-tab').each(function () {
            if ($(this).data('id') == dataUrl) {
                if (!$(this).hasClass('active')) {
                    ...
                    scrollToTab(this);
                    // 显示tab对应的内容区
                    $('.main-content .menu-iframe').each(function () {
                        if ($(this).data('id') == dataUrl) {
                            $(this).show().siblings('.menu-iframe').hide();
                            return false;
                        }
                    });
                }
        ...
        // 选项卡菜单不存在
        if (flag) {
            var str = '<a href="javascript:;" class="active menu-tab" data-id="' + dataUrl + '">' + menuName + ' <i class="fa fa-times-circle"></i></a>';
            $('.menu-tab').removeClass('active');

            // 添加选项卡对应的iframe
            var str1 = '<iframe class="menu-iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
            $('.main-content').find('iframe.menu-iframe').hide().parents('.main-content').append(str1);
    ...
    function closeTab() {
        ...
        if ($(this).parents('.menu-tab').hasClass('active')) {
            ...
            if ($(this).parents('.menu-tab').next('.menu-tab').length) {
    ...
    $('.tab-close-other').on('click', closeOtherTabs);

    //滚动到已激活的选项卡
    function showActiveTab() {
        scrollToTab($('.menu-tab.active'));
    }
    $('.tab-show-active').on('click', showActiveTab);
    ...
    $('.menu-tabs').on('click', '.menu-tab', activeTab);

    //刷新iframe
    function refreshTab() {
        var target = $('.menu-iframe[data-id="' + $(this).data('id') + '"]');
    ...
    $('.menu-tabs').on('dblclick', '.menu-tab', refreshTab);
